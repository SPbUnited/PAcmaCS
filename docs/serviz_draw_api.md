...
Каждый слой в `FeedData` имеет параметры `height` и `is_visible`, которые управляют отображением объектов.

### `height`

- Определяет порядок отрисовки слоев (аналог z-index).
- Слои сортируются по убыванию `height`.
- Чем больше `height`, тем выше слой рисуется.
...
- Порядок слоев также можно менять в интерфейсе (через изменение `height`).
